...
def infer_barcode_reverse_complement(barcode_whitelist, read_iter):
    ...
    N_bc_count = 0
    nbc = 0
    for _, seq, _ in itertools.islice(read_iter, cr_constants.NUM_CHECK_BARCODES_FOR_ORIENTATION):
        nbc += 1
        if "N" in seq:
            N_bc_count += 1
...
class ChunkedWriter(object):
    ...
    def generate_filename(self):
        raise NotImplementedError("generate_filename")
...
class ChunkedFastqWriter(ChunkedWriter):
    def __init__(self, *args, **kwargs):
        ...
        super(ChunkedFastqWriter, self).__init__(*args, **kwargs)
...
class FastqSpecException(Exception):
    def __init__(self, msg):
        super(FastqSpecException, self).__init__(msg)
        self.msg = msg
    def __str__(self):
        return self.msg
