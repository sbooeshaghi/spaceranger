...
from cellranger.rna.library import TARGET_SET_KEY

def get_cmd_names(product_name):
    """
    ...
    :param product_name: a string either "spaceranger" or "cellranger
    ...
    if product_name == "spaceranger":
        ...
        raise RuntimeError("I thought it was just the two of us, how can you do this to me?")
...
def load_csv_rownames(csv_file):
    rownames = np.atleast_1d(pd.read_csv(csv_file, usecols=[0], dtype=str, na_filter=False).values.squeeze())
    return rownames
...
def load_target_features_csv(features_csv):
    """Load a target feature CSV file (one integer index per line)"""
    with open(features_csv) as fh:
        return [int(x.rstrip()) for x in fh]

def load_target_feature_dict(libraries, target_features_csvs):
    """Combines a library_info with a map of target feature CSVs.
    Returns the integer representation"""
    r = collections.OrderedDict()
    for library, target_features in zip(libraries, target_features_csvs):
        r[library[TARGET_SET_KEY]] = load_target_features_csv(target_features)
    return r

def write_target_features_csv(path, feature_ints, header=None):
