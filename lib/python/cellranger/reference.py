    ...
    def write_genome_gtf(self, out_gtf_fn):
        with open(out_gtf_fn, 'wb') as f:
            ...
            for genome_prefix, in_gtf_fn in itertools.izip(self.genome_prefixes, self.in_gtf_fns):
                if len(self.genomes) > 1:
                    pfx = genome_prefix + '_'
                    prefix_func = lambda s, p=pfx: p + s
                else:
...
class GeneIndex(GtfParser):
    def __init__(self, in_gtf_fn=None, in_fasta_fn=None):
        """Constructor.
    ...
    def subset_index(self, target_genes):
        ...
        txs = {}
        for tx_id, tx in self.transcripts.iteritems():
            if tx.gene.id in target_genes:
                txs[tx_id] = tx
