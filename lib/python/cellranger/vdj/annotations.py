...
def setup_feature_aligners(reference_path, score_ratios, word_sizes, use_features=None):
    ...
    for region_types in VDJ_ORDERED_REGIONS:
        ...
        sc = score_ratios.get(region_types[0], VDJ_ANNOTATION_MIN_SCORE_RATIO)
        wd = word_sizes.get(region_types[0], VDJ_ANNOTATION_MIN_WORD_SIZE)
        mch = VDJ_ANNOTATION_MATCH_SCORE
        ...
        if region_types == VDJ_V_FEATURE_TYPES:
            feature_filter_params = lambda x, score=sc, match=mch: filter_v_alignment(x, score, match)
        else:
            feature_filter_params = lambda x, score=sc, word=wd, match=mch: filter_alignment(x, score, word, match)
...
class AnnotatedContig(object):
    ...
    def annotate_features_by_group(self, ssw_multi_aligner, alignment_filter):
        ...
        if top_annotations:
            ...
            for _, group in itertools.groupby(sorted(annotations, key=by_region), by_region):
