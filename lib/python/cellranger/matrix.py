...
from cellranger.feature_ref import FeatureReference, FeatureDef, GENOME_FEATURE_TAG
...
class CountMatrixView(object):
    ...
    def get_feature_indices_by_type(self, feature_type):
...
class CountMatrix(object):
    ...
    def save_h5_group(self, group):
        ...
        cr_io.create_hdf5_string_dataset(group, h5_constants.H5_BCS_ATTR, self.bcs, compression=True)
    ...
    @staticmethod
    def _load_dims_from_legacy_v1_h5_handle(f):
        # legacy format
        genomes = f.keys()
    ...
    @staticmethod
    def load_library_types_from_h5_file(filename):
        ...
        with h5.File(filename, 'r') as f:
            ...
            if version < MATRIX_H5_VERSION:
                ...
                if gene_count > 0:
                    return set([rna_library.GENE_EXPRESSION_LIBRARY_TYPE])
                else:
                    return set()
            else:
    ...
    def select_features(self, indices):
        ...
        feature_ref = FeatureReference(feature_defs = updated_feature_defs,
                                       ...
                                       target_features = self.feature_ref.target_features)
    ...
    def get_numfeatures_per_bc(self):
    ...
    def get_subselected_counts(self,
                                list_feature_ids=None,
                                list_barcodes=None,
                                log_transform=False,
                                library_type=None,
                                ):
        ...
        if library_type is not None:
            assert library_type in rna_library.RECOGNIZED_FEATURE_TYPES, "library_type not recognized"
        ...
        if log_transform:
            return np.log10(1. + counts_feature)
...
def make_library_map_aggr(gem_group_index):
    ...
    for _, (lid, og) in sorted(gem_group_index.iteritems(), key=lambda pair: int(pair[0])):
