...
class MoleculeCounter:
    ...
    def build_barcode_info(filtered_barcodes_by_genome, library_info, barcodes):
        ...
        if len(pf_tuples) > 0:
            pass_filter = np.array(
                pf_tuples, dtype=BARCODE_INFO_DTYPES['pass_filter'])
        else:
            pass_filter = np.zeros(
                (0, 3), dtype=BARCODE_INFO_DTYPES['pass_filter'])
    ...
    def get_filtered_barcodes(barcode_info, library_info, barcodes,
        ...
        lib_to_gg = np.array([lib['gem_group']
                              for lib in library_info], dtype='uint64')

        pf_gem_group = lib_to_gg[pf_library_idx[inds]]

        # Take unique, sorted barcodes (sorted by (gem_group, barcode_idx))
        gg_bcs = np.unique(np.column_stack(
            (pf_gem_group, pf_barcode_idx[inds])), axis=0)
    ...
    def open(filename, mode, feature_ref=None, barcodes=None, library_info=None,
        ...
        if mode == 'w':
            ...
            cr_io.set_hdf5_attr(
                mc.h5, h5_constants.H5_FILETYPE_KEY, MOLECULE_H5_FILETYPE)
            cr_io.set_hdf5_attr(mc.h5, FILE_VERSION_KEY, CURR_FILE_VERSION)
            ...
            mc.h5.create_dataset('barcodes', data=np.fromiter(
                barcodes, barcode_dtype, count=len(barcodes)), compression=HDF5_COMPRESSION)

            # Write library info
            lib_info_json = json.dumps(library_info, indent=4, sort_keys=True)
            cr_io.create_hdf5_string_dataset(
                mc.h5, 'library_info', [lib_info_json])
            ...
            if mc.file_version < CURR_FILE_VERSION:
                raise ValueError(
                    'The molecule info HDF5 file (format version %d) was produced by an older version of Cell Ranger. Reading these files is unsupported.' % mc.file_version)
            if mc.file_version > CURR_FILE_VERSION:
                raise ValueError(
                    'The molecule info HDF5 file (format version %d) was produced by a newer version of Cell Ranger. Reading these files is unsupported.' % mc.file_version)
            ...
            for key in mc.h5.keys():
                if key in MOLECULE_INFO_COLUMNS:
                    ...
                    mc.feature_reference = FeatureReference.from_hdf5(
                        mc.h5[key])
            ...
            # Load library info
            mc.library_info = json.loads(
                cr_io.read_hdf5_string_dataset(mc.h5['library_info'])[0])
    ...
    def get_chunks_from_partition_static(num_rows, values, key_func):
        ...
        starts = [
            0] + [MoleculeCounter.bisect_static(num_rows, val, key_func) for val in values[1:]]
        n = len(starts)
        for i in xrange(n):
    ...
    def get_chunks(self, target_chunk_len, preserve_boundaries=True):
        ...
        while chunk_end < (num_rows - 1):
            target_chunk_end = min(
                num_rows - 1, chunk_start + target_chunk_len - 1)
            chunk_end = self.find_last_occurrence_of_chunk_key(
                target_chunk_end) if preserve_boundaries else target_chunk_end
    ...
    def get_metrics_from_summary(summary, libraries, total_recovered_cells=None, total_force_cells=None):
        ...
        version_metrics = ['cellranger_version', 'reference_mkref_version',
                           'reference_fasta_hash', 'reference_gtf_hash']
        for m in version_metrics:
            if m in summary:
                mol_metrics[m] = summary[m]
            else:
                mol_metrics[m] = summary[rna_library.get_library_type_metric_prefix(
                    rna_library.ANTIBODY_METRIC_PREFIX) + m]
        ...
        for lib_idx, lib in enumerate(libraries):
            lib_type_prefix = rna_library.get_library_type_metric_prefix(
                lib[LIBRARY_TYPE])
            summary_name = '%s%s_total_read_pairs_per_library' % (
                lib_type_prefix, lib_idx)
        ...
        for gg in gem_groups:
            ...
            recovered_cells = total_recovered_cells / \
                len(gem_groups) if total_recovered_cells is not None else None
            force_cells = total_force_cells / \
                len(gem_groups) if total_force_cells is not None else None
            gg_metrics[str(gg)] = {
                GG_RECOVERED_CELLS_METRIC: recovered_cells,
                GG_FORCE_CELLS_METRIC: force_cells,
            }
    ...
    def get_raw_read_pairs_per_library(self):
        ...
        """
        try:
            return [self.get_metric(LIBRARIES_METRIC)[str(li)][TOTAL_READS_METRIC]
                    for li, _ in enumerate(self.library_info)]
        except KeyError:
    ...
    def get_usable_read_pairs_per_library(self):
        ...
        """
        try:
            return [self.get_metric(LIBRARIES_METRIC)[str(li)][USABLE_READS_METRIC]
                    for li, _ in enumerate(self.library_info)]
        except KeyError:
            raise RuntimeError('Missing metrics in molecule counter')

    def get_on_target_usable_read_pairs_per_library(self):
        """ Get usable on-target read pairs per library.
        Returns:
          list of int: Order is by library index
        """
        try:
            return [self.get_metric(LIBRARIES_METRIC)[str(li)][ON_TARGET_USABLE_READS_METRIC] for
                    li, _ in enumerate(self.library_info)]
        except KeyError:
            raise RuntimeError('Missing metrics in molecule counter')
    ...
    def convert_v2_to_v3(v2_mole_info_h5, out_v3_mole_info_h5):
        ...
        def get_chunks_by_gem_group(gem_group_arr):
            ...
            chunk_iter = MoleculeCounter.get_chunks_from_partition_static(
                num_rows, unique_ggs, gg_key)
        ...
        barcode_whitelist = cr_utils.load_barcode_whitelist(
            v2_metrics[BC_WHITELIST_METRIC])
        barcode_to_idx = OrderedDict((k, i)
                                     for i, k in enumerate(barcode_whitelist))
        ...
        # <-> feature_idx in v3
        v2_gene = np.asarray(
            v2_mc_in['gene'], dtype=MOLECULE_INFO_COLUMNS['feature_idx'])
        v2_conf_mapped_reads = np.asarray(
            v2_mc_in['reads'], dtype=MOLECULE_INFO_COLUMNS['count'])  # <-> count in v3
        # <-> transit into barcode_idx in v3
        v2_barcodes = np.asarray(v2_mc_in['barcode'], dtype=np.uint64)
        v2_umis = np.asarray(
            v2_mc_in['umi'], dtype=MOLECULE_INFO_COLUMNS['umi'])  # <-> umi in v3
        v2_gem_groups = np.asarray(
            v2_mc_in['gem_group'], dtype=MOLECULE_INFO_COLUMNS['gem_group'])  # <-> gem_group in v3
        ...
        for lib_idx, (gem_group, chunk_start, chunk_len) in enumerate(get_chunks_by_gem_group(v2_gem_groups)):
            ...
            recovered_cells = v2_metrics[GEM_GROUPS_METRIC][gem_group].get(
                GG_RECOVERED_CELLS_METRIC, None)
            force_cells = v2_metrics[GEM_GROUPS_METRIC][gem_group].get(
                GG_FORCE_CELLS_METRIC, None)
            ...
            for genome_id in v2_genome_ids:
                ...
                umis_per_bc = np.add.reduceat(
                    umi_conf_mapped_to_genome, bc_break_indices)

                if force_cells is not None:
                    top_bc_indices, _, _ = cellranger.cell_calling_helpers.filter_cellular_barcodes_fixed_cutoff(umis_per_bc, force_cells)
                else:
                    top_bc_indices, _, _ = cellranger.cell_calling_helpers.filter_cellular_barcodes_ordmag(umis_per_bc, recovered_cells, gg_total_diversity)

                # barcode info
                barcode_seq_to_idx = {b: barcode_to_idx[decompress_barcode_seq(
                    b, barcode_length)] for b in unique_bcs}

                barcode_info_genomes.append(genome_id)
                for b in unique_bcs[top_bc_indices]:
                    barcode_info_pass_filter.append(
                        (barcode_seq_to_idx[b], lib_idx, g_idx))

                # data
                barcode_idx_list.append(np.vectorize(
                    barcode_seq_to_idx.get)(bcs_for_genome))
                count_list.append(reads_for_genome)
                gem_group_list.append(np.full(
                    reads_for_genome.shape[0], gem_group, dtype=MOLECULE_INFO_COLUMNS['gem_group']))
                library_idx_list.append(np.full(
                    reads_for_genome.shape[0], lib_idx, dtype=MOLECULE_INFO_COLUMNS['library_idx']))
                feature_idx_list.append(gene_for_genome)
                umi_list.append(umis_for_genome)

        new_barcode_info = BarcodeInfo(
            pass_filter=np.array(barcode_info_pass_filter,
                                 dtype=BARCODE_INFO_DTYPES['pass_filter']),
        ...
        with MoleculeCounter.open(out_v3_mole_info_h5, 'w',
            ...
            out_mc.append_column(
                'barcode_idx', np.concatenate(barcode_idx_list))
            out_mc.append_column('count', np.concatenate(count_list))
            out_mc.append_column(
                'feature_idx', np.concatenate(feature_idx_list))
            ...
            out_mc.append_column(
                'library_idx', np.concatenate(library_idx_list))
