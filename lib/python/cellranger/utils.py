...
def _load_reference_metadata_file(reference_path):
    reference_metadata_file = os.path.join(
        reference_path, cr_constants.REFERENCE_METADATA_FILE)
...
def get_reference_gtf_path(reference_path):
...
def iter_by_qname(in_genome_bam, in_trimmed_bam):
    ...
    genome_bam_iter = itertools.groupby(
        in_genome_bam, key=lambda read: read.qname)

    if in_trimmed_bam is None:
        trimmed_bam_iter = iter(())
    else:
        trimmed_bam_iter = itertools.groupby(
            in_trimmed_bam, key=lambda read: read.qname)
...
def is_read_conf_mapped(read, high_conf_mapq):
    if read.is_unmapped:
        return False
...
def load_barcode_tsv(filename, as_set=False):
    barcodes = [x.strip() for x in cr_io.open_maybe_gzip(
        filename, 'r') if not ('#' in x)]
...
def get_barcode_whitelist_path(filename):
    ...
    if filename is None:
        ...
        gz = os.path.join(cr_constants.BARCODE_WHITELIST_PATH,
                          filename + '.txt.gz')
        if os.path.exists(gz):
            return gz

        txt = os.path.join(
            cr_constants.BARCODE_WHITELIST_PATH, filename + '.txt')
        return txt
...
def select_alignment_params(aln_params):
    ...
    if aln_params['aligner'] == 'star':
        set_if_none(result, 'high_conf_mapq',
                    cr_constants.STAR_DEFAULT_HIGH_CONF_MAPQ)
...
def is_metric_in_json(filename, key):
    with open(filename, 'r') as f:
        d = json.load(f)
    return key in d
...
def load_barcode_dist(filename, barcode_whitelist, gem_group, library_type, proportions=True):
    ...
    barcode_counts = {bc: value for bc, value in zip(
        barcode_whitelist, values[start:end])}
    if proportions:
...
def get_full_alignment_base_quality_scores(read):
    ...
    #pylint: disable=no-member
    quality_scores = np.fromstring(
        read.qual, dtype=np.byte) - tk_constants.ILLUMINA_QUAL_OFFSET
    ...
    for operation, length in read.cigar:
        ...
        if operation == 'D':
            ...
            quality_scores = np.delete(quality_scores, np.s_[
...
def get_unmapped_read_count_from_indexed_bam(bam_file_name):
    ...
    index_output = tk_subproc.check_output(
        'samtools idxstats %s' % bam_file_name, shell=True)
...
def string_is_ascii(input_string):
    """ Input strings are often stored as ascii in numpy arrays, and we need
    to check that this conversion works."""
    try:
        np.array(input_string.decode("utf-8"), dtype=str)
        return True
    except:
        return False


def get_read_iter_over_loci(loci, input_bam):
    """ Chain together BAM records from a list of loci so that the start
        position of each mapped read is contained in the interval [start, end)
        for each locus. input_bam is a pysam.AlignmentFile object.
    """
    for locus in loci:
        (chrom, start, end) = locus
        bam_iter = input_bam.fetch(chrom, start, end)
        for rec in bam_iter:
            if rec.is_unmapped or (rec.reference_start >= start and
                                   rec.reference_start < end):
                yield rec


def chunk_reference(input_bam, nchunks, include_unmapped):
    """ Chunk up a reference into nchunks roughly equally sized chunks.
        input_bam is only used to get the reference contig names and contig
        lengths and is a pysam.AlignmentFile object. Returns a list of loci,
        where each loci is a list of contiguous regions (contig, start, end).
        If include_unmapped is True then one of the chunks consists of all the
        unmapped reads as specified by [("*", None, None)].
    """
