...
def output_metrics_csv_from_annotations(metric_annotations, sample_data, out_fname, species_list=None):
    """
...
class MetricAnnotations(object):
    def __init__(self, metric_file=None):
        ...
        file_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), metric_file)
    ...
    def compile_summary_metrics(self, value_dict, species_list=None):
        ...
        Note: The Orion version of this returns a dictionary with key/value pairs, but this
        version will return a list of tuples with formatted-name/value
        """
        keylist = self.metric_data.keys()
        keys = [key for key in keylist if self.metric_data[key].summary]
    ...
    def gen_metric_list(self, value_dict, keys, species_list, debug=True):
        ...
        for key in keys:
            ...
            # not for spatial.
            if not self.metric_data.has_key(key):
                continue
            metric_info = self.metric_data[key]
                ...
                if is_barnyard and metric_info.include_cumulative:
                    ...
                    if subkey in value_dict:
                        output.append(self.gen_metric(
                            key, value_dict[subkey], debug=debug,
                            is_barnyard=is_barnyard, is_cumulative=True))
...
class SpatialMetricAnnotations(MetricAnnotations):
    def __init__(self):
        super(SpatialMetricAnnotations, self).__init__('spatial_metrics.csv')
