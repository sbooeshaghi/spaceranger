...
CI_LOWER_BOUND = (
    5.0
)  # CI lower bound (ie percentile value) for perturbation efficiencies
CI_UPPER_BOUND = (
    95.0
)  # CI upper bound (ie percentile value) for perturbation efficiencies
FILTER_LIST = [
    "None",
    "Non-Targeting",
    "Ignore",
...
MIN_NUMBER_CELLS_PER_PERTURBATION = (
    10
)  # Minimum number of cells a perturbation has to have before we compute differential expression for it
...
def save_top_perturbed_genes(
    ...
    for perturbation in results_per_perturbation:
        ...
        summary_df_columns += [
            "Perturbation: " + perturbation + ", " + s for s in column_map.values()
        ]
...
def sanitize_perturbation_results(res_table):
    ...
    res_table = res_table[
        (res_table["sum_a"] >= MIN_COUNTS_PERTURBATION)
        | (res_table["sum_b"] >= MIN_COUNTS_CONTROL)
    ]
    ...
    for this_row in protospacers_per_cell.itertuples():
        ...
        if this_row.num_features > 1:
            ...
            if ignore_multiples:
                ...
                this_target_ids = [
                    target_info.get(x).get("target_id") for x in this_features
                ]
                this_target_names = [
                    target_info.get(x).get("target_name") for x in this_features
                ]
                ...
                if set(this_target_ids) == set(control_list):
                    ...
                    this_target_ids = list(
                        set(this_target_ids).difference(set(filter_list))
                    )
                    this_target_names = list(
                        set(this_target_names).difference(set(filter_list))
                    )
                    if this_target_ids:
...
def _should_filter(
    ...
    target_id_name_map = _get_target_id_name_map(
        feature_ref_table, target_info, by_feature
    )
...
def get_perturbation_efficiency(
    ...
    matrix = feature_count_matrix.select_features_by_type(
    ...
    num_cells_per_perturbation = {
        name: sum(cluster_per_bc == cluster)
        for (cluster, name) in perturbation_keys.iteritems()
    }
...
def _analyze_transcriptome(
    ...
    filter_cluster_indices = [
        x for x in perturbation_keys if perturbation_keys[x] in filter_list
    ]
    ...
    nt_indices = [
        x for x in perturbation_keys if perturbation_keys[x] == "Non-Targeting"
    ]
    ...
    for cluster in perturbation_keys:
        ...
        all_de_results[:, 0 + 3 * (cluster_counter - 1)] = de_result["sum_a"] / len(
            group_a
        )
...
def _get_log2_fold_change(
    ...
    for (name, target) in itertools.izip(this_names, this_ids):
        ...
        log2_cis[name] = _get_fold_change_cis(
            matrix, target, group_a, group_b, local_params
        )
...
def _get_fold_change_cis(
    ...
    for i in xrange(num_bootstraps):
        ...
        log2_fold_change_vals[i] = np.log2(
            (1 + gene_sums_a) / (1 + size_factor_a)
...
def _get_target_id_name_map(feature_ref_table, target_info, by_feature=False):
    ...
    if not (by_feature):
        return {
            target_info[x]["target_name"]: target_info[x]["target_id"]
...
def _get_ps_clusters(
    ...
    """Returns a tuple (target_calls, perturbation_keys).
        ...
        perturbation_keys: dict - (cluster_number:perturbation_name) pairs
    """
    bc_targets_dict = _get_bc_targets_dict(
        target_info, protospacers_per_cell, ignore_multiples
    )
...
def _get_ps_clusters_by_target(
    ...
    target_id_name_map = {
        v.get("target_id"): v.get("target_name") for v in target_info.itervalues()
    }
