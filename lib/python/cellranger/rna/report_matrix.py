...
def _report_genome_agnostic_metrics(matrix, barcode_summary_h5, recovered_cells,
    ...
    cell_bcs_union = reduce(lambda a, x: a | set(
        x), cell_bc_seqs.itervalues(), set())
    ...
    d['%s_%s_total_raw_reads_per_filtered_bc' % (
        rna_library.MULTI_REFS_PREFIX, cr_constants.TRANSCRIPTOME_REGION)] = mean_reads_per_cell
    ...
    d['%s_%s_total_conf_mapped_reads_per_filtered_bc' % (
        rna_library.MULTI_REFS_PREFIX, cr_constants.TRANSCRIPTOME_REGION)] = tk_stats.robust_divide(total_conf_mapped_reads, n_cell_bcs_union)
    ...
    if genomes[0] != rna_library.MULTI_REFS_PREFIX:
        ...
        genome_matrices = OrderedDict(
            ((rna_library.MULTI_REFS_PREFIX, matrix),))
    ...
    if recovered_cells is None:
        d['%s_filtered_bcs_difference_from_recovered_cells' %
            rna_library.MULTI_REFS_PREFIX] = 0
        d['%s_filtered_bcs_relative_difference_from_recovered_cells' %
            rna_library.MULTI_REFS_PREFIX] = 0
    else:
        d['%s_filtered_bcs_difference_from_recovered_cells' %
            rna_library.MULTI_REFS_PREFIX] = int(n_cell_bcs_union) - int(recovered_cells)
        d['%s_filtered_bcs_relative_difference_from_recovered_cells' %
            rna_library.MULTI_REFS_PREFIX] = tk_stats.robust_divide(n_cell_bcs_union - recovered_cells, recovered_cells)
    ...
    for genome in genomes:
        ...
        for read_type in cr_constants.MATRIX_REPORT_READ_TYPES:
            ...
            if read_type in cr_constants.MATRIX_USE_MATRIX_FOR_READ_TYPE:
                ...
                n_reads = sum(
                    np.array(barcode_summary_h5[h5_key]).sum() for h5_key in h5_keys)
    ...
    for genome in genome_matrices.iterkeys():
    ...
    d['%s_transcriptome_usable_reads_frac' %
        rna_library.MULTI_REFS_PREFIX] = tk_stats.robust_divide(usable_reads, total_reads)
    ...
    reads_usable_per_cell = tk_stats.robust_divide(
        usable_reads, n_cell_bcs_union)
    d['%s_usable_reads_per_filtered_bc' %
        rna_library.MULTI_REFS_PREFIX] = reads_usable_per_cell
    # Create a feature barcoding dual whose name makes sense
    ...
    d['%s_filtered_gene_bc_matrix_density' % rna_library.MULTI_REFS_PREFIX] = tk_stats.robust_divide(
...
def _report(matrix, genome, barcode_summary_h5, recovered_cells, cell_bc_seqs,
    ...
    cell_bc_indices = _get_barcode_summary_h5_indices(
        barcode_summary_h5, cell_bc_seqs)
    ...
    top_genes_with_counts = {filtered_mat.int_to_feature_id(i): int(
        count) for i, count in cr_matrix.top_n(counts_per_gene, genes_top_n)}
    ...
    top_genes_with_unique_bcs = {filtered_mat.int_to_feature_id(i): int(
        count) for i, count in cr_matrix.top_n(unique_bcs_per_gene, genes_top_n)}
    ...
    if dupe_candidate_h5_key in barcode_summary_h5:
        ...
        n_reads = 0
        n_deduped_reads = 0
    d['filtered_bcs_%s_dupe_reads_frac' % cr_constants.CDNA_PCR_DUPE_TYPE] = 1 - \
        tk_stats.robust_divide(n_deduped_reads, n_reads)
...
def report_genomes(matrix, reads_summary, barcode_summary_h5_path, recovered_cells, cell_bc_seqs):
    ...
    for ftype in feature_types:
        ...
        genomes = matrix.get_genomes()
        conf_mapped_reads = _get_conf_mapped_reads(
            reads_summary, genomes, ftype)
        ...
        if rna_library.has_genomes(ftype):
            ...
            cell_bcs_union = list(
                reduce(lambda a, x: a | set(x), cell_bc_seqs.itervalues(), set()))
