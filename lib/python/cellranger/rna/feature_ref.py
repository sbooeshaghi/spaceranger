...
from cellranger.feature_ref import FeatureReference, FeatureDef, FeatureDefException, \
    GENOME_FEATURE_TAG, DEFAULT_FEATURE_TAGS, RESERVED_TAGS
...
PatternEntry = namedtuple(
    'PatternEntry', ['read', 'regex_string', 'regex', 'barcode_dict'])
...
FeatureMatchResult = namedtuple(
    'FeatureMatchResult', ['barcode', 'qual', 'ids', 'indices'])
...
class FeatureExtractor(object):
        ...
        for fd in feature_defs:
            ...
            if not pattern:
                raise FeatureDefException(
                    'Feature definition for %s has a sequence but no pattern specifying how to extract it.' % fd.id)
            if not read:
                raise FeatureDefException(
                    'Feature definition for %s has a sequence but no read specifying where to extract it from.' % fd.id)
            ...
            if pattern_key not in patterns:
                patterns[pattern_key] = PatternEntry(
                    read, regex_str, regex, {})
            entry = patterns[pattern_key]

            if sequence in entry.barcode_dict:
                raise FeatureDefException(
                    'Found two feature definitions with the same read ("%s"), pattern ("%s") and barcode sequence ("%s"). This combination of values must be unique for each row in the feature definition file.' % (read, pattern, sequence))
    ...
    def extract_paired_end(self, r1_seq, r1_qual, r2_seq, r2_qual):
        ...
        r1_any_whitelist, r1_matches = self._extract_from_seq(
            r1_seq, r1_qual, 'R1')
        r2_any_whitelist, r2_matches = self._extract_from_seq(
            r2_seq, r2_qual, 'R2')
...
def from_transcriptome_and_csv(gene_ref_path, feature_def_filename, target_features=None):
...
def compile_pattern(pattern_str, length):
    ...
    if not re.match(r'^\^{0,1}[ACGTN]*\${0,1}$', check_pattern):
        raise FeatureDefException(
            'Invalid pattern: "%s". The pattern must optionally start with "5P", optionally end with "3P", contain exactly one instance of the string "(BC)" and otherwise contain only the characters A, C, G, T, and N.' % input_pattern_str)
    ...
    try:
        ...
        raise FeatureDefException(
            'Failed to compile the feature definition string "%s" into a regular expression: %s.' % (regex_str, str(re.error)))
...
def parse_feature_def_file(filename, index_offset=0):
    ...
    for (row_num, row) in enumerate(reader):
        ...
        for key in row.iterkeys():
            if key is None:
                msg = "Your feature reference csv file contains more columns than the header on line %d. Please use a csv file with a header for each column." % (
                    row_num+2)
        ...
        if not (row['feature_type'] in ALLOWED_FEATURE_TYPES):
            msg_types = [x for x in ALLOWED_FEATURE_TYPES if x != rna_library.FEATURETEST_LIBRARY_TYPE]
            options = " or ".join(
                ("'%s'" % x for x in msg_types))
            msg = ("Unknown feature_type: '%s'."
                   "\nThe 'feature_type' field in the feature reference"
                   " must be one of %s") % \
                (row['feature_type'], options)
        ...
        if "\t" in row['name']:
            raise FeatureDefException(
                "Feature name field cannot contain tabs: '%s'" % row['name'])

        allowed_id_chars = set(string.printable) - \
        ...
        for (idx, c) in enumerate(row['id']):
            if not c in allowed_id_chars:
                if c in string.whitespace:
                    raise FeatureDefException(
                        u"Feature id field cannot contain whitespace: '%s'" % row['id'])
                else:
        ...
        tag_cols = [
            c for c in reader.fieldnames if c not in BASE_FEATURE_FIELDS and c in row]

        tags = OrderedDict()
        for key in tag_cols:
            if key in RESERVED_TAGS:
                raise FeatureDefException(
                    'Found invalid column name "%s." This name cannot be used as a custom feature tag because it is reserved for internal use.' % key)
        ...
        if len(tags['sequence']) == 0:
            raise FeatureDefException(
                'Found blank feature barcode sequence for feature id %s. The sequence column must be populated for this feature.' % row['id'])
        validate_sequence(tags['sequence'])

        if len(tags['pattern']) == 0:
            raise FeatureDefException(
                'Found blank feature barcode pattern for feature id %s. The pattern column must be populated for this feature.' % row['id'])
        compile_pattern(tags['pattern'], len(tags['sequence']))

        if tags['read'] not in ALLOWED_READS:
            raise FeatureDefException('The feature definition file contains a read type value "%s" which is not one of the allowed read types %s.' % (
