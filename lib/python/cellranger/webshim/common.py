...
def add_alarms(data, alarms_dict, alarms):
    ...
    if warn_dict is not None:
        add_alarm(value, formatted_value,
                  shared_constants.ALARM_WARN, warn_dict, alarms)
...
def build_tables(sample_properties, table_dicts, alarm_table_dicts, sample_data,
    ...
    for table_dict in table_dicts:
            ...
            if prefix is not None:
                ...
                if 'prefix_filter' in metric_dict:
                    if prefixes is not None:
                        prefixes = filter(
                            lambda x, md=metric_dict: x in md['prefix_filter'], prefixes)
                    else:
                        sys.stderr.write(
                            'Warning: no metric prefix values to filter for metric %s. The prefix name %s is probably wrong.\n' % (name, prefix))

                add_table_rows(sample_data.summary, name, metric_dict,
                               rows, style_func, target_func, prefixes=prefixes)
            else:
                add_table_rows(sample_data.summary, name,
                               metric_dict, rows, style_func, target_func)
        ...
        if (len(rows) == 0 and table_name == 'Mapping') or (len(rows) == 0 and table_name == 'Cells'):
            tables.append({
                'name': table_name,
                'rows': rows,
                'headers': metric_headers,
...
def build_plot_data_dict(plot_segment, counts):
    ...
    if plot_segment.cell_density > 0.:
        ...
        hover = "{:.0f}% Cells<br>({}/{})".format(100 *
                                                  plot_segment.cell_density, n_cells, n_barcodes)
    else:
        hover = "Background"
...
def plot_barcode_rank(chart, sample_properties, sample_data):
    ...
    if isinstance(sample_properties, dict):
        sample_properties = CountSampleProperties(sample_id=sample_properties.get('sample_id', None),
    ...
    if not isinstance(sample_properties, CountSampleProperties) or \
      sample_data.barcode_summary is None or \
      sample_data.cell_barcodes is None:
        return None
    ...
    ab_prefix = rna_library.get_library_type_metric_prefix(
        rna_library.ANTIBODY_LIBRARY_TYPE)
    gex_key = cr_utils.format_barcode_summary_h5_key(gex_prefix,
                                                     genome,
                                                     cr_constants.TRANSCRIPTOME_REGION,
                                                     cr_constants.CONF_MAPPED_DEDUPED_READ_TYPE)
    ab_key = cr_utils.format_barcode_summary_h5_key(ab_prefix,
...
def plot_clonotype_table(chart, sample_properties, sample_data):
    ...
    for name, col_def in col_defs.iteritems():
        if name not in clonotypes:
            raise ValueError(
                'Column not found in clonotype summary: %s' % name)
        cols.append({
...
def plot_barnyard_barcode_counts(chart, sample_properties, sample_data):
    ...
    call_to_series = {v['key']: i for i, v in enumerate(
        ws_gex_constants.GEM_CALL_LABELS)}
...
def plot_preprocess(analyses):
    ...
    sg_analyses = [an for an in analyses if isinstance(
        an, SingleGenomeAnalysis)]
...
def plot_dimensions(chart, transformed_matrix, description, pc1, pc2,
    ...
    for value in np.unique(values):
        chart['data'][value - 1] = {
            ...
            'y': [],
            'name': '%s %s - %s cells' % (
                description, format_value(value, 'integer'), format_value(
                    value_freqs[value], 'integer'),
            ),
            'hoverinfo': 'name',
            'mode': 'markers',
            'type': 'scattergl',
...
def plot_tsne_totalcounts(chart, sample_properties, sample_data):
    ...
    vmin, vmax = np.percentile(
        reads_per_bc, ws_gex_constants.TSNE_TOTALCOUNTS_PRCT_CLIP)
...
def clustering_plot_func(chart, sample_properties, sample_data, plot_func, args=[], kwargs={}):
    ...
    for clustering_key, clustering in analysis.clusterings.iteritems():
        ...
        if new_chart is not None:
            new_chart['filters'] = {
                ws_gex_constants.CLUSTERS_FILTER_TITLE: clustering.description}
...
def make_chart_filters(analyses):
    ...
    filter_values = map(lambda x: x.description, cr_clustering.sort_clusterings(
        analysis.clusterings.values()))
...
def plot_subsampled_scatterplot_metric(chart, sample_properties, sample_data, **kwargs):
    ...
    metric_search_results = [
        re.search(metric_pattern, key) for key in summary_data.keys()]
    ...
    if kwargs.get('multi_genome_only', False):
        references = set([result.group(1)
                          for result in metric_search_results if result is not None])
        if len(references) <= 2:
            return None
    ...
    if chart.get('layout') and chart['layout'].get('shapes') and chart['layout']['shapes'][0]['type'] == 'line':
        chart['layout']['shapes'][0]['x1'] = max(
            [max(trace['x']) for trace in traces.itervalues()])
...
def build_charts(sample_properties, chart_dicts, sample_data,
    ...
    for chart_dict in chart_dicts:
        ...
        if function is not None and f is None:
            raise ValueError(
                'Could not find webshim chart function "%s"' % function)
        ...
        new_charts = new_chart_obj if isinstance(
            new_chart_obj, list) else [new_chart_obj]
    ...
    for key, values in all_prefixes.iteritems():
        ...
        if values is not None and \
           any(v.startswith(chain_filter) for v in values):
            result[key] = [v for v in values if v.startswith(
                chain_filter) or v == rna_library.MULTI_REFS_PREFIX]
        else:
            result[key] = values
    return result
...
def get_custom_features(sample_data):
    ...
    if analysis and analysis.matrix.feature_ref.get_count_of_feature_type(rna_library.CUSTOM_LIBRARY_TYPE) > 0:
...
def get_genomes(sample_data):
    ...
    analysis = sample_data.get_analysis(SingleGenomeAnalysis)
    if analysis:
        return [x for x in analysis.matrix.get_genomes()
                if x != '']
    else:
        return ['dummy']
...
def build_web_summary_json(sample_properties, sample_data, pipeline):
    ...
    metrics, alarms, charts, all_prefixes = get_constants_for_pipeline(
        pipeline, sample_properties)
    ...
    all_prefixes['references'] = list(
        set(all_prefixes['references'] + get_genomes(sample_data)))
...
def build_info_dict(sample_properties, sample_data, pipeline):
    ...
    info['chemistry_description'] = sample_data.summary.get(
...
def build_web_summary_html(filename, sample_properties, sample_data, pipeline,
    ...
    with open(filename, 'w') as f:
        f.write(template.convert_webshim_json_to_html(
            view, pipeline, template_dir=template_dir))
    ...
    if alerts_output_filename is not None:
        with open(alerts_output_filename, 'w') as f:
            json.dump(tk_safe_json.json_sanitize(
                view.get('alarms', [])), f, indent=4, sort_keys=True)


def build_metrics_summary_csv(filename, sample_properties, sample_data, pipeline):
    metrics, alarms, _, all_prefixes = get_constants_for_pipeline(
        pipeline, sample_properties)
    ...
    tables, _ = build_tables(sample_properties_as_dict, metrics,
                             alarms, sample_data, all_prefixes=all_prefixes)
    if not tables:
        sys.stderr.write(
            "No metrics tables were generated, skipping CSV generation.\n")
        return
