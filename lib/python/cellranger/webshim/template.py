...
DEFAULT_TEMPLATE_DIR = os.path.join(os.path.dirname(__file__), "template")

def get_template_for_pipeline(pipeline, data):
    if pipeline == constants.PIPELINE_COUNT:
        is_barnyard = len(data['genomes']) > 1
        if is_barnyard:
            return 'count-multi-genome.html'
        else:
            return 'count-single-genome.html'
    elif pipeline == constants.PIPELINE_AGGR:
        is_barnyard = len(data['genomes']) > 1
        if is_barnyard:
            return 'aggr-multi-genome.html'
        else:
            return 'aggr-single-genome.html'
    elif pipeline == constants.PIPELINE_REANALYZE:
        return 'reanalyze.html'
    elif pipeline == constants.PIPELINE_VDJ:
        return 'vdj.html'
    elif pipeline == constants.PIPELINE_META_COUNT:
        return 'meta-multi-genome.html'
    else:
