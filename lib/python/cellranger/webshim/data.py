...
from cellranger.websummary.sample_properties import SampleProperties, CountSampleProperties
...
def get_plot_segment(start_index, end_index, sorted_bc, cell_barcodes, legend=False):
    ...
    num_cells = sum([1 for i in xrange(start_index, end_index)
...
class SampleData:
    def __init__(self, sample_properties, sample_data_paths, plot_preprocess_func):
        ...
        self.barcode_summary = load_barcode_summary(
            sample_data_paths.barcode_summary_path)

        self.vdj_clonotype_summary = pd.read_csv(
            sample_data_paths.vdj_clonotype_summary_path) if sample_data_paths.vdj_clonotype_summary_path else None

        self.vdj_barcode_support = pd.read_csv(
            sample_data_paths.vdj_barcode_support_path) if sample_data_paths.vdj_barcode_support_path else None

        self.cell_barcodes = cr_utils.get_cell_associated_barcode_set(
            sample_data_paths.filtered_barcodes_path) if sample_data_paths.filtered_barcodes_path else None

        if sample_data_paths.vdj_cell_barcodes_path:
            assert self.cell_barcodes is None
            self.cell_barcodes = set(vdj_utils.load_cell_barcodes_json(
    ...
    def counter_barcode_rank_plot_data(self, key):
        ...
        srt_order = compute_sort_order(
            counts_per_bc, self.barcode_summary['bc_sequence'][:], self.cell_barcodes)
        sorted_bc = self.barcode_summary['bc_sequence'][:][srt_order]
        sorted_counts = counts_per_bc[srt_order]
        del srt_order

        plot_segments = compute_plot_segments(
            sorted_bc, sorted_counts, self.cell_barcodes)
    ...
    def vdj_barcode_rank_plot_data(self):
        ...
        """
        counts_per_bc = self.vdj_barcode_support['count'].as_matrix()
        srt_order = compute_sort_order(
            counts_per_bc, self.vdj_barcode_support['barcode'], self.cell_barcodes)
        sorted_bc = self.vdj_barcode_support['barcode'].as_matrix()[srt_order]
        sorted_counts = counts_per_bc[srt_order]
        del srt_order
...
def compute_sort_order(counts_per_bc, bc_sequences, cell_barcodes):
    ...
    srt_order = sorted(range(len(counts_per_bc)), key=lambda x: (
        counts_per_bc[x], is_cell[x]), reverse=True)
    return srt_order
...
def compute_plot_segments(sorted_bc, sorted_counts, cell_barcodes):
    ...
    plot_segments = []
    plot_segments.append(BarcodeRankPlotSegment(
    ...
    mixed_segments = segment_log_plot_by_length(
        sorted_counts, ranges[1], ranges[2])
    for i in xrange(len(mixed_segments)-1):
        plot_segments.append(get_plot_segment(
