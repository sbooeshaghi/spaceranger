...
from tenkit.constants import SAMPLE_SINGLE_INDEX_MAP, SAMPLE_DUAL_INDEX_MAP
...
def rows_are_valid_csv(rows):
    ...
    if row_is_simple_header(rows[0]):
        ...
        logging.warning("The first row in the CSV file doesn't contain the required fields.\nPlease check for extra spaces or typos.")
        return False
...
def row_is_simple_dual_index(row):
    ...
    """
    return row_is_simple_header(row) \
        and row_get_col_index(row, 'index2') == 3
...
def row_is_simple_data(row):
    ...
    if not (row[2] in SAMPLE_SINGLE_INDEX_MAP or row[2] in SAMPLE_DUAL_INDEX_MAP or OLIGO_RE.match(row[2])):
...
def row_is_dual_index_data(row):
    ...
    another i5 oligo.
    """
    simple_data, err = row_is_simple_data(row)
    if err is not None:
        return False, err
...
def check_sample_index_collision(data_section, barcode_mismatches_param="1"):
    ...
    for sample_row in data_section.rows[1:]:
        ...
        if len(sample_row) <= lane_index or len(sample_row) <= si_index or (si2_index != -1 and len(sample_row) <= si2_index):
            continue 
        if not sample_row:
            continue
        lane_spec = "" # wildcard
    ...
    for idx, spec1 in enumerate(index_specs):
        for spec2 in index_specs[idx+1:]:
            ...
            if si_distance <= barcode_mismatches[0]*2:
                ...
                if si2_index == -1:
                    return (True, "%s, %s" % (spec1.index, spec2.index))
                else:
                    ...
                    if si2_distance <= barcode_mismatches[1]*2:
                        return (True, "%s, %s dual index" % (spec1.index2, spec2.index2))
...
def transform_data_section(section,
         i7_index_length=0,
         i5_index_length=0,
    ...
    if must_single_index_flowcell and si2_index != -1:
        raise SingleIndexFlowcellException("Dual-index configuration found for single-index flowcell")
    ...
    for input_row in section.rows[1:]:
        ...
        # found symbolic 4-oligo index; expand into multiple rows
            ...
            for idx, oligo in enumerate(oligos):
                ...
                elif i5_index_length:
                    raise DualIndexFlowcellException("Single-index 10x sample index used in dual-index flowcell")
...
def transform_samplesheet_sections(sections,
      r1_read_length=None,
      r2_read_length=None,
...
def transform_samplesheet(
    ...
    if len(data_section) > 0:
        ...
        (collides, message) = check_sample_index_collision(data_section[0], barcode_mismatches_param)
        if collides:
