...
def _load_libssw():
    """Load libssw.so using ctypes.
    
    `OSError` is converted to `ImportError`, to allow dependency analysis tools
    to correctly handle the error if the .so is not present.
    """
    so_file_name = os.path.dirname(os.path.abspath(__file__)) + '/' + "libssw.so"
    try:
        # Load the ssw library using ctypes
        return cdll.LoadLibrary(so_file_name)
    except OSError as err:
        raise ImportError("Error loading libssw.so: "+str(err))
