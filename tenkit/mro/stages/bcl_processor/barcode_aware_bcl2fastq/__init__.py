...
def split(args):
    ...
    if args.split_by_tile:
        chunk_defs = [
            {"tile_suffix": char, "__threads": 6, "__mem_gb": 16}
...
def process_raw_ilmn_data(args, outs):
    ...
    if major_ver == tk_bcl.BCL2FASTQ_V1:
        ...
        if ret > 0:
            martian.throw(
                "running the BclToFastq Makefile failed with code: %d. Exiting" % ret
            )
        elif ret < 0:
        ...
        if tile_split:
            proj_output_dir = os.path.join(
                output_dir, "Tile%s" % args.tile_suffix, "Project_%s" % flowcell
            )
        else:
    ...
    if tile_split:
        fastq_glob = os.path.join(
            output_dir, "Tile*", "Project_" + flowcell, "*", "*.fastq*"
        )
    else:
