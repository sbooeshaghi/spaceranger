...
def generate_whitelist_path_param(args):
    """
    Choose between barcode_whitelist and barcode_whitelist_candidates
    input params to generate either a single path or comma-separated path
    arg into the barcodeqc executable.
    """
    if args.barcode_whitelist:
        whitelist_candidates = args.barcode_whitelist.split(",")
        whitelists = [tk_preflight.check_barcode_whitelist(candidate) for candidate in whitelist_candidates]
        return ",".join(whitelists)
    else:
        # should have been caught by preflight, let fail in Go 
        return ""
...
def main(args, outs):
    ...
    whitelist_param = generate_whitelist_path_param(args)
    ...
    for idx, bf in enumerate(barcode_files):
        ...
        subproc_args = [ 'barcodeqc', bf.filename, output_json_path, "--whitelist", whitelist_param,
