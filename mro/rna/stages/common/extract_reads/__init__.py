...
def validate_concordance(r1, r2, bc, umi, si):
    """ A fastq file is corrupt if the reads in different files (which are supposed to be in order)
    have different names.  This method checks that if a read has a name present (some might be
    EMPTY_READS) that the names match. The method will take a series of tuples representing the
    (Name, Seq, Qual) of a set of reads (assuming no read is paired end.)

    A cython version of this code did not appear to be significantly faster, and both took
    about 10 micro seconds for the doc test below.

    >>> EMPTY_READ = (None, '', '')
    >>> r1 = (b"A00447:208:HL73LDMXX:2:2447:17870:4038/1 1:N:0:2", "", "")
    >>> r2 = (b"A00447:208:HL73LDMXX:2:2447:17870:4038/2 2:Y:0:2", "", "")
    >>> bc = EMPTY_READ
    >>> si = EMPTY_READ
    >>> umi = EMPTY_READ
    >>> validate_concordance(r1, r2, bc, si, umi)
    """
    # Chop off the read number element bit:
    # https://help.basespace.illumina.com/articles/descriptive/fastq-files/
    reads = [x[0].split(" ")[0] for x in [r1, r2, bc, umi, si] if x[0]]
    if reads:
        exp = reads[0][:-1]
        for read in reads[1:]:
            if read[:-1] != exp:
                raise AssertionError(("Position paired FASTQ reads have different names, {} vs {}."
                                      " Check the input FASTQ files for corruption.").format(
                    reads[0], read))
...
def main(args, outs):
    ...
    for extractions in itertools.islice(all_read_iter, args.chunk_initial_reads):
        ...
        validate_concordance(rna_read, rna_read2, bc_read, si_read, umi_read)
