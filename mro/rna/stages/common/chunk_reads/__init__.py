...
def main(args, outs):
    ...
    try:
        tk_subproc.check_output(chunk_reads_args)
    except CalledProcessError as e:

        chnk = args.read_chunk["read_chunks"]
        filenames = ""
        ...
        if e.returncode == 2:
            msg = "Malformed FASTQ data detected in one of these inputs:\n"
            msg += filenames
            msg += e.output
            martian.exit(msg)
        else:
            msg = "Error processing FASTQ files:\n"
            msg += filenames
            msg += e.output
            print msg
            martian.exit(msg)
