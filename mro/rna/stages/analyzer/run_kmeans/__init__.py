...
def split(args):
    ...
    max_clusters = args.max_clusters if args.max_clusters is not None else analysis_constants.MAX_N_CLUSTERS_DEFAULT
    ...
    matrix_mem_gb = np.ceil(MEM_FACTOR * cr_matrix.CountMatrix.get_mem_gb_from_matrix_h5(args.matrix_h5))
...
def join(args, outs, chunk_defs, chunk_outs):
    ...
    analysis_io.combine_h5_files(chunk_h5s, outs.kmeans_h5, [analysis_constants.ANALYSIS_H5_CLUSTERING_GROUP,
                                                       analysis_constants.ANALYSIS_H5_KMEANS_GROUP])

    for csv_dir in chunk_csv_dirs:
        cr_io.copytree(csv_dir, outs.kmeans_csv, allow_existing=True)
