...
def select_barcodes_and_features(matrix, is_antibody_only, num_bcs=None, use_bcs=None, use_genes=None,
                                 exclude_genes=None, force_cells=None):

    if force_cells is not None:
        bc_counts = matrix.get_counts_per_bc()
        bc_indices, _, _ = cell_calling_helpers.filter_cellular_barcodes_fixed_cutoff(bc_counts, force_cells)
        with LogPerf('f1'):
    ...
    if is_antibody_only:
        num_features = matrix.get_count_of_feature_type(rna_library.ANTIBODY_LIBRARY_TYPE)
    else:
        num_features = matrix.get_count_of_feature_type(rna_library.GENE_EXPRESSION_LIBRARY_TYPE)
    if num_features < PCA_N_COMPONENTS_DEFAULT:
        err_msg = ("To run the analysis, you must specify at least {} 'Gene Expression' or 'Antibody Capture' features."
                   "You only have {} features in this data.  Check that the 'exclude_genes' and "
...
def join(args, outs, chunk_defs, chunk_outs):
    ...
    with LogPerf('select'):
        matrix = select_barcodes_and_features(matrix, args.is_antibody_only,
    ...
    with LogPerf('w1'):
        matrix.save_h5_file(outs.cloupe_matrix_h5, extra_attrs=matrix_attrs, sw_version=martian.get_pipelines_version())

    with LogPerf('selnz'):
        matrix, _, _ = matrix.select_nonzero_axes()

    with LogPerf('w2'):
        matrix.save_h5_file(outs.preprocessed_matrix_h5, extra_attrs=matrix_attrs, sw_version=martian.get_pipelines_version())
