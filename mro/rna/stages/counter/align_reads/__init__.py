...
def join(args, outs, chunk_defs, chunk_outs):
    outs.genome_output = sum([co.genome_output for co in chunk_outs], [])
...
def align_reads(args, outs):
    ...
    genome_output = martian.make_path('genome_output.bam')
    outs.genome_output = [genome_output]

    star.align(args.read_chunk, args.read2_chunk, genome_output,
...
def create_unaligned_bam(args, outs):
    ...
    genome_output = martian.make_path('genome_output.bam')
    outs.genome_output = [genome_output]
    outbam = pysam.AlignmentFile(genome_output, 'wb', template=samfile)
