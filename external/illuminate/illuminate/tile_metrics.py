...
class InteropTileMetrics(InteropBinParser):
    ...
    def parse_binary(self):
        ...
        # is still incomplete.
        try:
            self.aligned = pivot_mean.loc[300].values[0]
        except:
            self.aligned = 0

        try:
            self.total_cluster_density = pivot_sum.loc[100].values[0]
        except:
            self.total_cluster_density = 0

        try:
            self.total_cluster_density_pf = pivot_sum.loc[101].values[0]
        except:
        ...
        try:
            self.num_clusters = pivot_sum.loc[102].values[0]
        except:
            self.num_clusters = 0

        # SAV: "PF Reads"
        try:
            self.num_clusters_pf = pivot_sum.loc[103].values[0]
        except:
        ...
        for read in self.read_config:
            ...
            try:
                self.mean_phasing.append(pivot_mean.loc[200 + (read['read_num']-1) * 2].values[0])
                self.mean_prephasing.append(pivot_mean.loc[201 + (read['read_num']-1) * 2].values[0])
            except:
